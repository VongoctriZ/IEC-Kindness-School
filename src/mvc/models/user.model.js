 ...
 * @property {number}   cyclePoints      — điểm trong chu kỳ hiện tại (0..999), dùng cho thanh tiến độ
 * @property {number}   matureTreeCount  — số lần đã harvest Cây trưởng thành
 * @property {import('firebase/firestore').Timestamp} createdAt
 */

export function buildUserDoc(firebaseUser, extra = {}) {
  const role = extra.role ?? ROLES.STUDENT
  const startPoints = role === ROLES.PENDING_TEACHER ? 0 : POINTS.REGISTER
  return {
